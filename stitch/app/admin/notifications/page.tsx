...
export default async function AdminNotificationsPage({ searchParams }: { searchParams?: Promise<{ success?: string; error?: string }> }) {
  ...
  return (
    <div className="space-y-8 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="relative z-10 flex flex-col justify-between gap-8 md:flex-row md:items-end">
          <div>
            <div className="inline-flex items-center gap-2.5 rounded-full border border-blue-200 bg-blue-50/80 px-4 py-1.5 shadow-sm backdrop-blur-md">
              ...
              <span className="text-[11px] font-black uppercase tracking-[0.2em] text-blue-700">Trung tâm thông báo</span>
            </div>
            <h1 className="mt-5 font-headline text-4xl font-black tracking-tight text-slate-900 md:text-5xl">Notification Center</h1>
            <p className="mt-3 max-w-2xl text-base text-slate-500 font-medium leading-relaxed">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="group flex flex-col items-center justify-center rounded-3xl border border-white bg-white/60 p-5 shadow-sm backdrop-blur-xl transition-all hover:scale-105 hover:bg-white hover:shadow-md">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-amber-100 to-orange-100 text-amber-600 shadow-sm">
                  <span className="material-symbols-outlined text-xl">mark_email_unread</span>
                </div>
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Chưa đọc</p>
            ...
            <div className="group flex flex-col items-center justify-center rounded-3xl border border-white bg-white/60 p-5 shadow-sm backdrop-blur-xl transition-all hover:scale-105 hover:bg-white hover:shadow-md">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-cyan-100 to-blue-100 text-blue-600 shadow-sm">
                  <span className="material-symbols-outlined text-xl">notifications</span>
                </div>
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Tổng thư</p>
            ...
            <div className="group flex flex-col items-center justify-center rounded-3xl border border-white bg-white/60 p-5 shadow-sm backdrop-blur-xl transition-all hover:scale-105 hover:bg-white hover:shadow-md">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-emerald-100 to-teal-100 text-emerald-600 shadow-sm">
                  <span className="material-symbols-outlined text-xl">admin_panel_settings</span>
                </div>
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Tài khoản</p>
                  <p className="max-w-[120px] truncate font-headline text-sm font-black text-slate-900" title={meEmail || "—"}>{meEmail || "—"}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="px-2">
        {sp?.success === "send" ? (
          <div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50/80 px-5 py-4 text-emerald-900 shadow-sm backdrop-blur-md">
            <span className="material-symbols-outlined text-emerald-600">check_circle</span>
            <p className="text-sm font-medium">Đã gửi thông báo thành công tới các người dùng đã chọn.</p>
          </div>
        ) : null}
        {sp?.error === "validation" ? (
          <div className="flex items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50/80 px-5 py-4 text-rose-900 shadow-sm backdrop-blur-md">
      ...
      <div className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="flex flex-col gap-6">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-3">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-100 text-blue-600 shadow-sm backdrop-blur-md">
                <span className="material-symbols-outlined">edit_square</span>
              </span>
              <h2 className="font-headline text-2xl font-black text-slate-800">Soạn thông báo</h2>
            </div>
            <span className="rounded-full border border-blue-200 bg-blue-50 px-4 py-1.5 text-xs font-bold text-blue-700 shadow-sm backdrop-blur-sm">
              {users.length} người dùng hệ thống
            </span>
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-3">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-100 text-cyan-600 shadow-sm backdrop-blur-md">
                <span className="material-symbols-outlined">inbox</span>
              </span>
              <h2 className="font-headline text-2xl font-black text-slate-800">Hộp thư hệ thống</h2>
            </div>
            <p className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-bold text-slate-500 shadow-sm">
              Live updates
            </p>
          </div>
          <div className="overflow-hidden rounded-[2.5rem] border border-white bg-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl">
            <div className="max-h-[850px] overflow-y-auto p-4 sm:p-6 custom-scrollbar">

    ...
    <div className="space-y-8">
      ...
      <section className="flex flex-col justify-between gap-6 overflow-hidden rounded-3xl border border-slate-200/50 dark:border-slate-800/50 bg-white/60 dark:bg-slate-900/60 p-6 shadow-xl shadow-purple-900/5 dark:shadow-none backdrop-blur-xl md:flex-row md:items-end">
        <div>
          ...
          <h1 className="mt-3 font-headline text-4xl font-black tracking-tight text-purple-900 dark:text-purple-400">Banner Quảng Cáo</h1>
          ...
          <table className="w-full min-w-[800px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              {currentBanners.map((b, idx) => {
                ...
                return (
                  <tr key={b.id} className="group transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                    <td className="px-6 py-4 text-right font-mono text-xs text-slate-500 dark:text-slate-400">{startIndex + idx + 1}</td>
                    <td className="px-6 py-4">
                      {b.imageUrl ? (
                        <img src={resolveCatalogImageUrl(b.imageUrl)} alt={b.title || "banner"} className="h-12 w-24 rounded-lg object-cover shadow-sm border border-slate-200 dark:border-slate-700" />
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      <p className="font-headline text-sm font-bold text-blue-900 dark:text-white">{b.title || "—"}</p>
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 max-w-[200px] truncate" title={b.linkUrl || ""}>

...
import { getAdminBanner } from "@/lib/sale-api"; // Need to add this to sale-api.ts
...
export default async function EditBannerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // We actually don't have getAdminBanner in sale-api yet, let's assume we will add it.
  const banner = await getAdminBanner(bannerId, { accessToken: session.token });
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Chỉnh sửa thông tin banner quảng cáo hiện tại.
          </p>
        </div>
      </div>

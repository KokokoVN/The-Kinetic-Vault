    ...
    <div className="space-y-8">
      <section className="flex flex-col justify-between gap-6 overflow-hidden rounded-3xl border border-white/40 bg-white/60 p-6 shadow-xl shadow-blue-900/5 backdrop-blur-xl md:flex-row md:items-end">
        <div>
          ...
          <h1 className="mt-3 font-headline text-4xl font-black tracking-tight text-blue-900">Quản lý Tồn kho</h1>
          <p className="mt-2 max-w-2xl text-slate-500">
            Quản lý và cập nhật số lượng hàng hóa tồn kho
          </p>
        </div>
        <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white/60 px-5 py-3 backdrop-blur-sm shadow-sm">
          <span className="material-symbols-outlined text-2xl text-blue-700">warehouse</span>
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Tổng mã SP</p>
            <p className="font-headline text-2xl font-black text-blue-900">{initialBalances.totalElements}</p>
          </div>
        </div>
      </section>

...
export default function EditBrandForm({ brand, gatewayOrigin }: { brand: AdminBrand; gatewayOrigin: string }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      setSubmitError("Xóa thương hiệu thất bại. Có thể thương hiệu đang được sử dụng bởi sản phẩm.");
    }
  };
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Cập nhật thông tin cho thương hiệu <span className="font-bold text-slate-700 dark:text-slate-300">#{brand.id}</span>.
            </p>
          </div>
        </div>
        ...
        <form action={handleDelete}>
          <ClientConfirmSubmitButton
            ...
            className="group flex items-center gap-2 rounded-xl bg-red-50 dark:bg-red-500/10 px-4 py-2 text-sm font-bold text-red-600 dark:text-red-400 transition-colors hover:bg-red-100 dark:hover:bg-red-500/20"
            confirmMessage={`Bạn có chắc chắn muốn xóa thương hiệu "${brand.name}"? Hành động này không thể hoàn tác.`}
          >
            <span className="material-symbols-outlined text-[18px] transition-transform group-hover:scale-110">delete_forever</span>
            Xóa thương hiệu
          </ClientConfirmSubmitButton>
        </form>
      </div>
            ...
            <div>
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 p-6 text-center backdrop-blur-sm transition-colors hover:bg-slate-50/80 dark:hover:bg-slate-800/80">
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  >
                    {brand.logoUrl ? "Thay đổi ảnh..." : "Tải ảnh lên"}
                  </label>

...
public class OrderJwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private static final Pattern ORDER_POST_USER = Pattern.compile("^/order/(\\d+)$");

    private final OrderJwtProperties properties;
    private final AntPathMatcher pathMatcher = new AntPathMatcher();

    public OrderJwtAuthenticationFilter(OrderJwtProperties properties) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        final Claims claims;
        try {
            claims = Jwts.parser().setSigningKey(properties.getSecret()).parseClaimsJws(token).getBody();
        } catch (Exception e) {
        ...
        if ("POST".equalsIgnoreCase(request.getMethod())) {
            Matcher m = ORDER_POST_USER.matcher(path);
            if (m.matches()) {
                long pathUserId = Long.parseLong(m.group(1));
                Object uidObj = claims.get("uid");
                if (uidObj == null) {
                    writeJson(response, HttpServletResponse.SC_UNAUTHORIZED, "Token thiếu uid");
                    return;
                }
                long tokenUid = ((Number) uidObj).longValue();
                if (tokenUid != pathUserId) {
                    writeJson(response, HttpServletResponse.SC_FORBIDDEN,
                            "userId trong URL không khớp với token");
                    return;
                }
            }
        }
    ...
    private static void writeJson(HttpServletResponse response, int status, String message) throws IOException {
        ...
        String body = "{\"error\":\"" + (status == 403 ? "Forbidden" : status == 503 ? "Service Unavailable" : "Unauthorized")

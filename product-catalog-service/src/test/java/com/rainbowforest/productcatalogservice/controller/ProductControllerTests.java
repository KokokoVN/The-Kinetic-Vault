...
//        given(productService.getAllProduct()).willReturn(products);
    ...
    @Test
    public void get_available_products_for_user_should_return200_when_validRequest() throws Exception {
        when(productService.getAllAvailableProductForUser()).thenReturn(products);
        ...
        verify(productService, times(1)).getAllAvailableProductForUser();
        verifyNoMoreInteractions(productService);
    }

    @Test
    public void get_available_products_for_user_should_return404_when_emptyList() throws Exception {
        when(productService.getAllAvailableProductForUser()).thenReturn(new ArrayList<>());
    ...
    @Test
    public void get_available_products_by_category_should_return200_when_validRequest() throws Exception {
        when(productService.getAllAvailableProductForUserByCategoryId(PRODUCT_ID)).thenReturn(products);

        mockMvc.perform(get("/products/available/category/{categoryId}", PRODUCT_ID))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$[0].id").value(PRODUCT_ID));

        verify(productService, times(1)).getAllAvailableProductForUserByCategoryId(PRODUCT_ID);
        verifyNoMoreInteractions(productService);
    }

    @Test
    public void get_newest_products_should_return200_when_validRequest() throws Exception {
        when(productService.getNewestAvailableProductsForUser(8)).thenReturn(products);
        ...
        verify(productService, times(1)).getNewestAvailableProductsForUser(8);
        verifyNoMoreInteractions(productService);
    }
    ...
    public void get_all_product_by_category_controller_should_return200_when_validRequest() throws Exception {
        ...
        mockMvc.perform(get("/products").param("category", PRODUCT_CATEGORY))
                ...
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$[0].id").value(PRODUCT_ID))
                .andExpect(jsonPath("$[0].categoryId").value(PRODUCT_ID));
    ...
    public void get_one_product_by_id_controller_should_return200_when_validRequest() throws Exception {
        ...
        mockMvc.perform(get("/products/{id}", PRODUCT_ID))
                ...
                .andExpect(jsonPath("$.categoryId").value(PRODUCT_ID));
